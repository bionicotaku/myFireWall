...
int showRules(struct IPRule *rules, int len) {
	int i;
	if(len == 0) {
		printf("rule list is empty\n");
		return 0;
	}
	//printf("rule num: %d\n", len);
	printLine(111);
	printf("| %-*s | %-18s | %-18s | %-11s | %-11s | %-8s | %-6s | %-3s |\n", MAXRuleNameLen,
	 "name", "source IP", "dest IP", "source port", "dest port", "protocol", "action", "log");
	printLine(111);
	for(i = 0; i < len; i++) {
		showOneRule(rules[i]);
	}
...
int showOneLog(struct IPLog log) {
	...
	// print
	printf("[%s] %-9s %s -> %s protocol:%s length:%u\n",
		tm, action, saddr, daddr, proto, log.len/10);
}
...
int showConns(struct ConnLog *logs, int len) {
	...
	printLine(col);
	printf("| %-8s | %-22s |->| %-22s | %-9s |\n", "protocol", "source IP", "dest IP", "status");
	printLine(col);
	for(i = 0; i < len; i++) {
		showOneConn(logs[i]);
	}
